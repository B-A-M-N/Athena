...
privilege needed for the workspace. The supported stable host is Linux; macOS
and Windows are compatibility targets, not parity claims. Agent code executes
only through the configured capability/execution policy and workspace sandbox
boundaries, but operators should still treat a project workspace as sensitive.
