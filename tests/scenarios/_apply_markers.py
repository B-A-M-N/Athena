...
MARKS = {
    (
        "tests/unit/models/test_role_router.py",
        "test_router_exposes_selected_provider_without_second_authority",
    ): ["FUSE-001"],
    (
        "tests/unit/service/test_direct_execution.py",
        "test_direct_execution_routes_through_dispatcher_and_records_audit",
    ): ["FUSE-003"],
    ("tests/unit/kernel/test_kernel_loop.py", "test_end_to_end_simple_completes"): ["FUSE-004"],
    (
        "tests/unit/kernel/test_kernel_loop.py",
        "test_scripted_capability_then_answer_runs_two_iterations",
    ): ["FUSE-004"],
    ...
    (
        ...
        "test_validate_reports_invalid_generated_schema_as_admission_failure",
    ): ["SYNTH-001"],
    ...
    (
        "tests/unit/synthesis/test_synthesis.py",
        "test_register_ephemeral_registers_validated_and_invokes",
    ): ["SYNTH-002"],
    (
        "tests/unit/synthesis/test_synthesis.py",
        "test_generated_authority_is_sandbox_profile_not_declared_effects",
    ): ["SYNTH-003"],
    ...
    (
        ...
        "test_model_can_create_task_local_tool",
    ): ["SYNTH-005"],
    (
        "tests/unit/capabilities/test_synthesis_capability.py",
        "test_synthesis_generates_strict_input_schema_from_fixtures",
    ): ["SYNTH-005"],
    (
        "tests/unit/capabilities/test_synthesis_capability.py",
        "test_synthesis_output_schema_inference_distinguishes_booleans",
    ): ["SYNTH-005"],
    (
        "tests/unit/capabilities/test_synthesis_capability.py",
        "test_synthesis_promotion_is_policy_checked",
    ): ["AUTH-001"],
    (
        "tests/unit/capabilities/test_synthesis_capability.py",
        "test_synthesis_requires_task_scope",
    ): ["AUTH-001"],
    (
        "tests/unit/affordances/test_fabric.py",
        "test_restore_executor_rechecks_source_and_output_contract",
    ): ["AUTH-002"],
    (
        "tests/unit/affordances/test_fabric.py",
        "test_restore_executor_rejects_persisted_source_escape",
    ): ["AUTH-002"],
    (
        "tests/unit/affordances/test_fabric.py",
        "test_restore_executor_rejects_persisted_authority_outside_profile",
    ): ["AUTH-002"],
    (
        "tests/unit/capabilities/test_scratch.py",
        "test_scratch_rejects_host_escape_before_execution",
    ): ["AUTH-003"],
    (
        "tests/unit/affordances/test_validation.py",
        "test_source_validation_rejects_host_escape_primitives_before_execution",
    ): ["AUTH-003"],
    (
        "tests/security/test_policy_bypass.py",
        "test_task_deny_is_hard_ceiling_even_when_global_allows",
    ): ["AUTH-004"],
    ...
    (
        ...
        "test_unknown_required_field_is_invalid_not_invented",
    ): ["COMPAT-004"],
    ...
    (
        "tests/unit/models/test_toolcall_candidates.py",
        "test_empty_candidate_is_not_rewritten_as_an_empty_object",
    ): ["COMPAT-004"],
    ...
    (
        ...
        "test_preflight_repaired_arguments_are_used",
    ): ["COMPAT-006"],
    (
        "tests/unit/capabilities/test_tool_repair_store.py",
        "test_parallel_repair_receipt_is_durable_and_replayable",
    ): ["COMPAT-006"],
    (
    ...
    (
        ...
        "test_accumulator_merges_mixed_stream_without_duplicate_text",
    ): ["COMPAT-001"],
    ...
    (
        "tests/unit/capabilities/test_screen_emulator.py",
        "test_overwrite_moves_cursor_and_replaces",
    ): ["BODY-002"],
    ...
    (
        ...
        "test_claims_go_stale_after_dependent_mutation",
    ): ["CLAIM-001"],
    (
        "tests/unit/shadow/test_shadow_and_worldstate.py",
        "test_shadow_commit_applies_proven_changes",
    ): ["TX-001"],
    (
        "tests/unit/shadow/test_shadow_and_worldstate.py",
        "test_shadow_discard_leaves_reality_untouched",
    ): ["TX-002"],
    (
        "tests/unit/fusion/test_orchestrator.py",
        "test_experiment_commit_binds_claim_and_invalidates",
    ): ["CLAIM-002"],
    ("tests/unit/fusion/test_orchestrator.py", "test_failed_criteria_discards_and_auto_forks"): [
        "TX-005"
    ],
    (
        "tests/unit/kernel/test_continuation_store.py",
        "test_claim_is_not_consumed_until_execution_finishes",
    ): ["CLAIM-003"],
    ...
    (
        "tests/unit/capabilities/test_watch.py",
        "test_file_watch_detects_same_size_edit_with_preserved_timestamp",
    ): ["ENV-003"],
    ...
    (
        "tests/crash/test_scheduler_recovery.py",
        "test_claimed_occurrence_reconciled_to_fired_on_restart",
    ): ["RECOVERY-003"],
    (
        "tests/unit/cli/test_oi_stream.py",
        "test_oi_viewer_renders_to_configured_output_and_keeps_partials",
    ): ["PROJECTION-001"],
    ("tests/unit/cli/test_oi_stream.py", "test_oi_viewer_does_not_write_to_process_stdout"): [
        "PROJECTION-001"
    ],
    (
        "tests/unit/cli/test_surface.py",
        "test_direct_shell_escape_uses_the_same_execution_surface",
    ): ["PROJECTION-002"],
}
    ...
    for rel, marks in sorted(by_file.items()):
        ...
        for idx, line in enumerate(lines):
            ...
            if m and m.group(2) in marks and m.group(2) not in inserted:
                ...
                prev = next((lines[j] for j in range(idx - 1, -1, -1) if lines[j].strip()), "")
