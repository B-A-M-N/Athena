...
async def test_memory_recalled_in_session_but_not_other(make_service):
    ...
    await _seed_session_memory(svc, session_a, f"{_MARKER} is a durable fact about session A")
...
async def test_memory_capability_save_binds_session_and_returns_bounded_json(make_service):
    ...
    raw = await svc._db.fetch_one("SELECT metadata FROM memories WHERE id = ?", (record.id,))
...
async def test_memory_capability_isolates_sessions_tasks_and_projects(make_service):
    ...
    await _dispatch_memory(
        ...
        workspace=workspace,
        operation="save",
        content="session A private marker",
        scope="session",
    )
    await _dispatch_memory(
        svc,
...
async def test_context_compiler_retrieves_capability_written_memory(make_service):
    ...
    await _dispatch_memory(
        ...
        workspace=workspace,
        operation="save",
        content=f"{_MARKER} is a compiler-visible fact about the marker",
        scope="session",
    )
