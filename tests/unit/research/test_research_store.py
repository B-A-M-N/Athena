...
async def test_sources_evidence_and_gaps_survive_store_roundtrip(tmp_path):
    ...
    source = SourceRecord.for_uri(
        "artifact://sha256/abc", title="captured", content_hash="abc",
        artifact_uri="artifact://sha256/abc", task_id="task-1",
    )
    ...
    gap = ResearchGap.create(
        "check the value", "Is the captured value 42?", task_id="task-1")
...
async def test_indexed_source_content_is_searchable_and_scoped(tmp_path):
    ...
    own = SourceRecord.for_uri(
        "artifact://sha256/own", content_hash="own", task_id="task-1",
    )
    project = SourceRecord.for_uri(
        "artifact://sha256/project", content_hash="project", project_id="repo",
    )
    foreign = SourceRecord.for_uri(
        "artifact://sha256/foreign", content_hash="foreign", task_id="task-2",
    )
    ...
    hits = await store.search_content(
        "repair", task_id="task-1", project_id="repo", limit=10,
    )
    ...
    project_hits = await store.search_content(
        "procedure", task_id="task-1", project_id="repo", limit=10,
    )
