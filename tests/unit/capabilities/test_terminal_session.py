...
async def test_create_send_screen_kill(term, tmp_path):
    ...
    r = await term.invoke(_req("create", task_id="t1", command="bash --norc"), context=context)
    ...
    await term.invoke(
        _req("send", task_id="t1", session=sid, text="echo done-tag-xyz"), context=context
    )
    r = await term.invoke(
        _req("wait_for", task_id="t1", session=sid, pattern="done-tag-xyz", timeout=5),
        context=context,
    )
...
async def test_task_ownership_enforced(term, tmp_path):
    ...
    await term.invoke(_req("create", task_id="t1", command="bash --norc"), context=context)
