...
class GeneratedCapabilityStore:
    ...
    async def save(self, capability: GeneratedCapability, *, owner: str) -> None:
        ...
        if (capability.scope is AffordanceScope.CANDIDATE
                and capability.task_scope not in (None, owner)):
        ...
        promoting_candidate = (
            ...
            and capability.scope in {
                AffordanceScope.PROJECT, AffordanceScope.USER,
            }
            and str(
                capability.provenance.get("task_id")
                or capability.task_scope
                or ""
            ) == existing["owner"]
        )
    ...
    async def get(
        ...
        if row["scope"] == AffordanceScope.USER.value \
            ...
            return None
        if row["scope"] == AffordanceScope.CANDIDATE.value \
                and task_id != row["owner"]:
            return None
    ...
    async def list(
        ...
        for row in rows:
            ...
            if row["scope"] == AffordanceScope.USER.value \
                ...
                continue
            if row["scope"] == AffordanceScope.CANDIDATE.value \
                    and row["owner"] != task_id:
                continue
            out.append(capability)
        return out

    async def transition(
        self, capability_id: str, lifecycle_state: str, *,
        owner: str | None = None, reason: str = "",
    ) -> GeneratedCapability:
        ...
        if owner:
            ...
            params.append(owner)
        row = await self._db.fetch_one(
            "SELECT definition FROM generated_capabilities WHERE "
        ...
        await self._db.execute(
            ...
            "updated_at = ? WHERE id = ?",
            (json.dumps(definition, sort_keys=True), enabled,
             utcnow().isoformat(), capability_id),
        )
        return GeneratedCapability.from_record(definition)

    async def find_equivalent(
        self, capability: GeneratedCapability, *, owner: str,
    ) -> GeneratedCapability | None:
    ...
    async def disable(self, capability_id: str, *, owner: str | None = None) -> bool:
        ...
        history.append({
            ...
            "to": "DEPRECATED", "reason": "deprecated",
            "at": utcnow().isoformat(),
        })
    ...
    async def garbage_collect(self, *, before: str | None = None) -> int:
        ...
        if before is None:
            cursor = await self._db.execute(
                "DELETE FROM generated_capabilities WHERE enabled = 0"
            )
        else:
            cursor = await self._db.execute(
                "DELETE FROM generated_capabilities WHERE enabled = 0 "
