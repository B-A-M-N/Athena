...
from athena.presentation.code_view import CodeViewport, make_code_view
...
def build_oi_scene(
    ...
    *,
    ...
    # Live scene identity never resurrects prior work. History views can use
    # ``last_operation_id`` explicitly.
    active = state.operations.get(state.active_operation_id) if state.active_operation_id else None
    try:
    ...
    elif state.status in {"FAILURE", "BLOCKED"}:
    ...
    # Completed code mutations remain live presentation context for the
    # current operation even after mutation state advances. If no operation is
    # active, fall back only to the most recent operation for this dedicated
    # artifact view—not for live action identity above.
    code_operation = active or (
        state.operations.get(state.last_operation_id) if state.last_operation_id else None
    )
    code_view = None
    if code_operation and (code_operation.content_preview or code_operation.diff_preview):
        code_view = make_code_view(
            path=code_operation.target or code_operation.label,
            text=code_operation.content_preview or "\n".join(code_operation.diff_preview),
            mutation_state=code_operation.mutation_state,
            diff_hunks=code_operation.diff_preview,
            preview_truncated=code_operation.preview_truncated,
        )
