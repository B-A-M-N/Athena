...
def classify_event(
    ...
    # These are recoverable facts about a child attempt, not terminal task
    # facts.  Keep the visual identity in the relevant activity family while
    # the projection records attention for the failed attempt.
    if event == "modelrequestfailed":
        return VisualActionKind.THINK
    if event == "verificationcompleted":
        return VisualActionKind.VERIFY
    if event in {"executiontimedout", "executioninterrupted"}:
        return VisualActionKind.EXECUTE
    if event == "capabilityfailed":
        return classify_visual_action(
            data.get("capability_id") or data.get("capability") or data.get("runtime"),
            operation=data.get("operation") or data.get("action") or args.get("operation"),
            arguments=args,
        )
    return classify_visual_action(
        data.get("capability_id") or data.get("capability") or data.get("runtime"),
