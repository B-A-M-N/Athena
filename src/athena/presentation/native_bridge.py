...
from athena.presentation.scene import TreeNode, build_oi_scene
...
def native_projection_frame(
    ...
    *,
    ...
    conversation: list[dict[str, Any]] | tuple[dict[str, Any], ...] | None = None,
) -> ProjectionFrame:
    ...
    # Live projection must not resurrect prior work. ``last_operation_id``
    # remains available to explicit history views only.
    ...
    frame: ProjectionFrame = {
        ...
        "conversation": [
            {
                ...
                "role": sanitize_terminal_text(item.get("role", "user")),
                "text": sanitize_terminal_text(item.get("text", "")),
            }
            for index, item in enumerate((conversation or [])[-128:])
        ],
        "status": sanitize_terminal_text(state.status),
        "status_message": sanitize_terminal_text(state.status_message),
        "failure": failure_envelope(
            sanitize_terminal_text(state.failure_info.message or state.failure_reason),
            sanitize_terminal_text(state.failure_info.stage or state.failure_stage),
            kind=sanitize_terminal_text(state.failure_info.kind),
            code=sanitize_terminal_text(state.failure_info.code),
            fatal=state.failure_info.fatal,
        ).as_dict(),
        ...
        # One canonical visual vocabulary crosses the seam. Rust may map this
        # serialized value to renderer-local enums, but it must not choose
        # between view/buddy/event fields and invent a different mode.
        "visual_mode": sanitize_terminal_text(scene.mode.value),
